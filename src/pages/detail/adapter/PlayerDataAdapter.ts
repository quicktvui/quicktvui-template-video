...
import {IMediaUrl} from "../../../datasource/media/IMediaUrl";
...
//--------------------------------------播放器数据-------------------------------------------------
export function buildMediaItemList(page: number, mediaList: Array<IMedia>, mediaItemInterceptor: ESIPlayerInterceptor): Array<ESMediaItem> {
  const list = mediaList;
  const itemList: Array<ESMediaItem> = []
  if (list && list.length) {
    for (let i = 0; i < list.length; i++) {
      const m = list[i]
      const item = buildMediaItem(page, i, m, mediaItemInterceptor)
      itemList.push(item)
    }
  }
  return itemList
...
export function buildMediaItem(page: number, index: number, media: IMedia, mediaItemInterceptor: ESIPlayerInterceptor): ESMediaItem {
  let mediaIndex = Number(page * 10) + Number(index)
  let mediaItem: ESMediaItem = {
    id: media.id,
    index: mediaIndex,
    title: media.title,
    interceptors: [mediaItemInterceptor]
  }
  return mediaItem
}
