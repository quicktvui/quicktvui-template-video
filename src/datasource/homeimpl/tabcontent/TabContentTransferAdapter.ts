...
import {buildEndSection} from "../../../pages/home/page/page";
 ...
 * @param isFirstPage
 */
export function buildTransferTabContentAdapter(tabContent:any,isFirstPage:boolean=false):QTTabPageData{
  //定义板块总高度
  ...
  if (plateSourceData && plateSourceData.length>0){
    plateSourceData.map((plateItem,plateIndex:number)=>{
      ...
      const tabPlateData:TabContentPlate = buildPlateData(
        ...
        plateHeight,showPlateName,false,plateItem.plateType, tabSectionList)
      //封装板块
...
function buildPlateHeight(plateItem):number{
  ...
  if (showPlateName == '1'){//展示
    plateHeight += (tabPlateTitleHeightDefault + tabPlateTitleGap)// 60:默认文字高度，30：间距
  }
  return plateHeight
}
