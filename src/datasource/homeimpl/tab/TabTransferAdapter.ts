...
import {
  buildQTTab,
  buildTabImgItem,
  buildTabTextItem, tabBackgroundUrls,
  tabDecorationGapLeft,
  tabDecorationGapRight,
  tabSectionTitleSize
} from "./TabAdapter";
import TabType from "./TabType/TabType";

/**
 * tab数据转换
 * @param tabs
 */
export function buildTransferTabAdapter(tabs:Array<any>):QTTab{
  ...
  tabs.map((tab,index)=>{
    ...
    const decoration: ESListViewItemDecoration = {
      left: index === 0 ? tabDecorationGapLeft : 0,
...
function buildTabItem(tab:any,decoration?: ESListViewItemDecoration):QTTabItem{
  ...
  switch (menuType) {
    case TabType.TAB_TEXT_TYPE:
      tabItem = buildTabTextItem(tab.menuCode,tab.menuName,tabSectionTitleSize,QTTabItemType.QT_TAB_ITEM_TYPE_DEFAULT,decoration)
      break;
    case TabType.TAB_IMG_TYPE:
      ...
      break;
    default:
      tabItem = buildTabTextItem(tab.menuCode,tab.menuName,44,QTTabItemType.QT_TAB_ITEM_TYPE_DEFAULT,decoration)
      break;
  }
  return tabItem
}
